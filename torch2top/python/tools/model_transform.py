...
class TorchModelTransformTool(ModelTransformTool):  
    def __init__(self, model_name, onnx_model, input_shapes:list=[]):
        super().__init__(model_name)
        self.onnx_model = onnx_model
        self.input_shapes = input_shapes
    ...
    def _transform_(self, mlir_file):
        cvt = TorchConverter(self.model_name, self.onnx_model, self.input_shapes, [], [], mlir_file)
        cvt.run()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_type", required=False, choices=['onnx'], help="model_type")
    ...
    if args.model_def.endswith('.pt'):
        tool = TorchModelTransformTool(args.model_name, args.model_def, args.input_shapes)

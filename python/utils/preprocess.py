...
def add_preprocess_parser(parser):
    ...
    parser.add_argument("--pixel_format", choices=['rgb', 'bgr', 'gray', 'rgba'], default='rgb',
...
class preprocess(object):
    ...
    def load_config(self, input_op):
        ...
        self.input_name = Operation.str(attrs['name'])
        self.pixel_format = Operation.str(attrs['pixel_format']) if 'pixel_format'in attrs else 'rgb'
        self.channel_num = 3
        if self.pixel_format == 'gray':
            self.channel_num = 1
        elif self.pixel_format == 'rgba':
            self.channel_num = 4
        self.channel_format = Operation.str(attrs['channel_format']) if 'pixel_format'in attrs else 'nchw'
        if self.channel_format == 'nhwc':
            self.net_input_dims = shape[1:-1]
        self.keep_aspect_ratio = Operation.bool(attrs['keep_aspect_ratio']) if 'keep_aspect_ratio'in attrs else False
        self.pad_value = Operation.int(attrs['pad_value']) if 'pad_value'in attrs else 0
        self.pad_type = Operation.str(attrs['pad_type']) if 'pad_type'in attrs else "center"
        self.resize_dims = Operation.int_array(attrs['resize_dims']) if 'resize_dims'in attrs else [224,224]

        if len(self.resize_dims) == 0:
            self.resize_dims = self.net_input_dims
        self.mean = np.array(Operation.fp_array(
            attrs['mean'])).astype(np.float32) if 'mean'in attrs else np.array([127.5, 127.5, 127.5])
        self.mean = self.mean[np.newaxis, :, np.newaxis, np.newaxis]
        self.scale = np.array(Operation.fp_array(
            attrs['scale'])).astype(np.float32) if 'scale'in attrs else np.array([0.00784313725490196, 0.00784313725490196, 0.00784313725490196])
        self.scale = self.scale[np.newaxis, :, np.newaxis, np.newaxis]
        self.crop_method = 'center'
        self.has_pre = True
